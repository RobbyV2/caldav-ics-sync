...
const getColorScheme = () => {
  return window.getComputedStyle(root_).getPropertyValue('color-scheme').toString()
}
...
for (const a of CollapseTriggers) {
  ...
  a.addEventListener('click', () => {
    if (t.classList.contains('show'))
      (t.classList.remove('show'), (t.style.height = ''), a.setAttribute('aria-expanded', !1))
    else {
...
for (const a of DropdownTriggers) {
  const b = a.getAttribute('data-win-target'),
    ...
    h = e => {
      c.contains(e.target) || a.contains(e.target) || !c.classList.contains('show') || g()
    }
  a.addEventListener('click', e => {
    ;(getScreenOffset(a) ? c.classList.add('reverse') : c.classList.remove('reverse'),
...
for (const d of TxtClearers) {
  ...
  ;(f.addEventListener('input', () => {
    '' !== f.value ? (d.style.visibility = 'visible') : (d.style.visibility = 'hidden')
  }),
...
for (const a of ModalTriggers) {
  ...
  a.onclick = function (t) {
    ;(t.target !== this &&
      (a.classList.contains('app-alert') || a.classList.contains('app-dialog'))) ||
...
for (const a of NavBarTogglers) {
  ...
  function toggleNavBarMobile() {
    ;(null === document.querySelector('.app-navbar-overlay') &&
      ((NavBarOverlay.className = 'app-navbar-overlay'), b.appendChild(NavBarOverlay)),
      b.classList.toggle('collapsed-float'),
      NavBarOverlay.classList.toggle('show'))
  }
  function toggleNavBar() {
    ;(window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) < 760
      ? toggleNavBarMobile()
...
function applyFill(e) {
  var l = (100 * (e.value - e.min)) / (e.max - e.min),
    l = `linear-gradient(90deg, ${SliderSettings.fill} ${l}%, ${SliderSettings.background} ${0.1 + l}%)`
